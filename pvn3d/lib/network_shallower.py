...
from pvn3d.lib.pointnet import PointNetfeat, ModifiedPointNetfeat
...
class ConvFeat(nn.Module):
    def __init__(self):
        ...
        self.conv1 = torch.nn.Conv1d(864, 512, 1)
        self.conv2 = torch.nn.Conv1d(512, 256, 1)
        self.conv3 = torch.nn.Conv1d(256, 128, 1)
...
class SGKPNet(nn.Module):
    def __init__(self, num_points, num_obj, num_kps):
        ...
        self.pntf = ModifiedPointNetfeat(global_feat=False, feature_transform=False)
    ...
    def forward(self, img, x, choose):
        ...
        rgb_emb = torch.cat([rgb_emb1, rgb_emb2], dim=1) # 32+256=288

        x = x.transpose(2, 1).contiguous()
        d_emb, trans, trans_feat = self.pntf(x) # 576 # 1088
        rgbd_emb = torch.cat([d_emb, rgb_emb], dim=1) # 576 + 288 = 864
        ...
        x_nm = self.conv_nm(x_nm).view(bs, 3, self.num_points)
        x_nm = x_nm.permute(0, 2, 1).contiguous()

        return x_seg, x_kp, x_c, trans, trans_feat
